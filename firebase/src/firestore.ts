...
export function collectionUpdater<T extends DocumentData>(collection: CollectionReference<T>, next: Next): Unsubscribe {
	const cache: Map<string, DocumentWithId<T>> = new Map()
	const unsub = snapshotSubscriber(collection, cache, next)
	return unsub
}

export function queryUpdater<T extends DocumentData>(query: Query<T>, next: Next): Unsubscribe {
	const cache: Map<string, DocumentWithId<T>> = new Map()
	const unsub = snapshotSubscriber(query, cache, next)
	return unsub
}

export function docUpdater<T extends DocumentData>(doc: DocumentReference<T>, next: Next): Unsubscribe {
